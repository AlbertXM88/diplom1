...
Процедура ВКМ_ОтправкаУведомлений() Экспорт
	...
	Запрос.Текст =
		"ВЫБРАТЬ
		|	ВКМ_УведомлениеТелеграммБоту.ТекстСообщения,
		|	ВКМ_УведомлениеТелеграммБоту.Ссылка,
		|	ВКМ_ТокенУправленияТелеграммБотом.Значение КАК Токен,
		|	ВКМ_ИдентификаторГруппыДляОповещения.Значение КАК ИдГруппы
		|ИЗ
		|	Справочник.ВКМ_УведомленияТелеграммБоту КАК ВКМ_УведомлениеТелеграммБоту,
		|	Константа.ВКМ_ТокенУправленияТелеграммБотом КАК ВКМ_ТокенУправленияТелеграммБотом,
	...
	Пока Выборка.Следующий() Цикл
		...
		ДанныеТГ = Новый Структура();
        ДанныеТГ.Вставить("Токен",Выборка.Токен);
        ДанныеТГ.Вставить("ИдГруппы",Выборка.ИдГруппы);
        
        Ответ = ВКМ_ИнтеграцияСТелеграммБотам.ОтправкаСообщенияВГруппу(Выборка.ТекстСообщения,ДанныеТГ);
