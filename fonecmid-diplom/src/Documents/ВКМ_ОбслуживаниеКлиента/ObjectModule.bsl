...
Процедура ОбработкаПроведения(Отказ,Режим)
	...
	ДанныеДоговораОбслуживания = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ДоговорКонтрагента,
	  "ВидДоговора,ВКМ_ДатаНачалаДоговора,ВКМ_ДатаОкончанияДоговора,ВКМ_СтоимостьЧасаРаботы" );
...
Процедура ПриЗаписи(Отказ)
	...
	Если   ЗнчРеквизитовОбъектаВБазе = Неопределено   Тогда
		...
		ТекстУведомления = СтрШаблон("Новая  задача от %1 .Выполнить: %2. Дата и время выполнения: %3 с %4 до %5.Исполнитель : %6",
		                             Клиент,ОписаниеПроблемы,Формат(ДатаПроведенияРабот,"ДФ=dd.MM.yyyy;" ),
		                             Формат(ВремяНачалоРабот,"ДЛФ=T;" ),Формат(ВремяОкончанияРабот,"ДЛФ=T;" ),Специалист);
	Иначе
